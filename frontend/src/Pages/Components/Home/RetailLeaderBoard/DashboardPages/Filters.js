import React, { useState, useEffect } from "react";
import DatePicker from "react-datepicker";
import "react-datepicker/dist/react-datepicker.css";
import "./Filters.css";
import { useHistory } from "react-router-dom";

import backIcon from "../../../../../assets/ImpactTracker/Back_Icon.svg";
import arrowIcon from "../../../../../assets/ImpactTracker/nav-arrow-icon.svg";
import filterIcon from "../../../../../assets/ImpactTracker/filter.png";

import "./CustomDatePicker.css";

const demoData = {
  regions: ["North", "South", "East", "West"],
  stores: {
    North: ["Store #1001 - NY", "Store #1002 - NJ"],
    South: ["Store #2001 - TX", "Store #2002 - FL"],
    East: ["Store #3001 - MA", "Store #3002 - CT"],
    West: ["Store #4001 - CA", "Store #4002 - WA"],
  },
  channels: ["E-commerce", "Wholesale", "Retail", "Direct", "Franchise"],
};

const subCategoryOptions = {
  "Produce (Fresh)": ["Fruits & Vegetables", "Herbs", "Organic Produce"],
  "Dry Goods": ["Grains", "Pasta", "Canned Goods"],
  "General Merchandising": ["Bakery", "Deli", "Household"],
};
export const Filters = ({ onApply, selectedCategory, setSelectedCategory }) => {
  const history = useHistory();
  const [region, setRegion] = useState(demoData.regions[0]);
  const [hoveredRegion, setHoveredRegion] = useState(null);
  const [isTimePeriodOpen, setIsTimePeriodOpen] = useState(false);

  const [selectedStores, setSelectedStores] = useState([]);
  const [selectedSubCategory, setSelectedSubCategory] = useState(
    subCategoryOptions[selectedCategory]?.[0] || ""
  );

  const [selectedChannels, setSelectedChannels] = useState([]);
  const [selectedDate, setSelectedDate] = useState(null);

  const applyFilters = (updatedFilters = {}) => {
    onApply({
      region,
      selectedStores,
      selectedChannels,
      selectedDate,
      category: selectedCategory,
      subCategory: selectedSubCategory,
      ...updatedFilters,
    });
  };

  const categoryOptions = [
    "Produce (Fresh)",
    "Dry Goods",
    "General Merchandising",
  ];

  useEffect(() => {
    setSelectedSubCategory(subCategoryOptions[selectedCategory]?.[0] || "");
  }, [selectedCategory]);

  useEffect(() => {
    setSelectedStores([]);
  }, [region]);

  const toggleCheckbox = (value, currentValues, setter) => {
    if (currentValues.includes(value)) {
      setter(currentValues.filter((v) => v !== value));
    } else {
      setter([...currentValues, value]);
    }
  };

  const PlainDropdown = ({ selectedValue, onSelect, options }) => {
    const [open, setOpen] = useState(false);

    return (
      <div className="relative">
        <button
          className="flex items-center text-sm text-black"
          onClick={() => setOpen(!open)}
        >
          <span>{selectedValue}</span>
          <img
            src={arrowIcon}
            alt="arrow"
            className="ml-1 w-3 h-3 transform transition-transform duration-200"
            style={{ transform: open ? "rotate(180deg)" : "rotate(0deg)" }}
          />
        </button>

        {open && (
          <div className="absolute z-10 bg-white border rounded mt-1 w-max shadow text-black font-normal">
            {options.map((option) => (
              <div
                key={option}
                onClick={() => {
                  onSelect(option);
                  setOpen(false);
                }}
                className="px-3 py-1 text-sm hover:bg-gray-100 cursor-pointer whitespace-nowrap"
              >
                {option}
              </div>
            ))}
          </div>
        )}
      </div>
    );
  };

  return (
    <div className="">
      <div className="filter-bar">
        <div className="flex items-center space-x-2 pl-5">
          <button className="arrow-btn" onClick={() => history.push("/Home")}>
            <img src={backIcon} alt="Back" />
          </button>

          <PlainDropdown
            selectedValue={selectedCategory}
            onSelect={(newCategory) => {
              setSelectedCategory(newCategory);
              const firstSubCat = subCategoryOptions[newCategory]?.[0] || "";
              setSelectedSubCategory(firstSubCat);
              applyFilters({ category: newCategory, subCategory: firstSubCat });
            }}
            options={categoryOptions}
          />
        </div>

        <div className="flex items-center space-x-2 flex-wrap">
          {/* Region Dropdown */}
          {/* Global Filter Icon */}
          <img src={filterIcon} alt="filter-icon" className="filter-bar-icon" />
          <Dropdown label="Region" hoverHighlight selectedValue={region}>
            {demoData.regions.map((r) => {
              const isSelected = region === r;
              const isHovered = hoveredRegion === r;

              const shouldHighlightOrange =
                isHovered || (!hoveredRegion && isSelected);

              return (
                <div
                  key={r}
                  className={`region-option px-3 py-1 text-xs cursor-pointer rounded ${
                    shouldHighlightOrange ? "text-orange-500" : "text-white"
                  }`}
                  onClick={() => {
                    setRegion(r);
                    applyFilters({ region: r });
                  }}
                  onMouseEnter={() => setHoveredRegion(r)}
                  onMouseLeave={() => setHoveredRegion(null)}
                >
                  {r}
                </div>
              );
            })}
          </Dropdown>

          {/* Store Dropdown */}
          <Dropdown
            label="Store"
            selectedValue={
              selectedStores.length
                ? selectedStores.length === 1
                  ? selectedStores[0]
                  : `${selectedStores.length} selected`
                : "Store"
            }
          >
            {demoData.stores[region].map((store) => (
              <div key={store} className="dropdown-option">
                <input
                  type="checkbox"
                  checked={selectedStores.includes(store)}
                  onChange={() => {
                    toggleCheckbox(store, selectedStores, setSelectedStores);
                    applyFilters({
                      selectedStores: selectedStores.includes(store)
                        ? selectedStores.filter((v) => v !== store)
                        : [...selectedStores, store],
                    });
                  }}
                />
                <span>{store}</span>
              </div>
            ))}
          </Dropdown>

          {/* Time Period Dropdown */}
          <Dropdown
            label="Time Period"
            selectedValue={
              selectedDate ? selectedDate.toLocaleDateString() : "Time Period"
            }
            isCalendar
          >
            <div className="calendar-container">
              <DatePicker
                selected={selectedDate}
                onChange={(date) => {
                  setSelectedDate(date);
                  applyFilters({ selectedDate: date });
                }}
                inline
                calendarClassName="custom-datepicker"
                showPopperArrow={false}
                dayClassName={(date) =>
                  date.getMonth() !== (selectedDate || new Date()).getMonth()
                    ? "faded-day"
                    : undefined
                }
              />
            </div>
          </Dropdown>

          <Dropdown
            label="Sub-Category"
            selectedValue={selectedSubCategory || "Sub-Category"}
          >
            {subCategoryOptions[selectedCategory]?.map((subCat) => (
              <div
                key={subCat}
                className="dropdown-option"
                onClick={() => {
                  setSelectedSubCategory(subCat);
                  applyFilters({ subCategory: subCat });
                }}
              >
                <input
                  type="radio"
                  checked={selectedSubCategory === subCat}
                  onChange={() => setSelectedSubCategory(subCat)}
                  name="sub-category"
                />
                <span>{subCat}</span>
              </div>
            ))}
          </Dropdown>

          {/* Store Channel Dropdown */}
          <Dropdown
            label="Store Channel"
            selectedValue={
              selectedChannels.length
                ? selectedChannels.length === 1
                  ? selectedChannels[0]
                  : `${selectedChannels.length} selected`
                : "Store Channel"
            }
          >
            {demoData.channels.map((channel) => (
              <div key={channel} className="dropdown-option">
                <input
                  type="checkbox"
                  checked={selectedChannels.includes(channel)}
                  onChange={() => {
                    toggleCheckbox(
                      channel,
                      selectedChannels,
                      setSelectedChannels
                    );
                    applyFilters({
                      selectedChannels: selectedChannels.includes(channel)
                        ? selectedChannels.filter((v) => v !== channel)
                        : [...selectedChannels, channel],
                    });
                  }}
                />
                <span>{channel}</span>
              </div>
            ))}
          </Dropdown>

          {/* Apply/Clear Buttons */}
          <div className="flex space-x-2 ml-2">
            {/* <button
              className="btn-apply"
              onClick={() => {
                onApply({
                  region,
                  selectedStores,
                  selectedChannels,
                  selectedDate, // changed from dateRange
                  category: selectedCategory,
                  subCategory: selectedSubCategory,
                });
              }}
            > 
              Apply
            </button> */}
            <button
              className="btn-clear"
              onClick={() => {
                const defaultRegion = demoData.regions[0];
                const defaultCategory = "Produce (Fresh)";
                const defaultSubCategory =
                  subCategoryOptions[defaultCategory]?.[0] || "";

                setRegion(defaultRegion);
                setSelectedStores([]);
                setSelectedChannels([]);
                setSelectedDate(null);
                setSelectedCategory(defaultCategory);
                setSelectedSubCategory(defaultSubCategory);

                applyFilters({
                  region: defaultRegion,
                  selectedStores: [],
                  selectedChannels: [],
                  selectedDate: null,
                  category: defaultCategory,
                  subCategory: defaultSubCategory,
                });
              }}
            >
              Clear
            </button>
          </div>
        </div>
      </div>
    </div>
  );
};

const Dropdown = ({
  label,
  children,
  selectedValue,
  hoverHighlight = false,
  isCalendar = false,
}) => (
  <div className="dropdown group relative">
    <button className="dropdown-button">
      <span className="dropdown-label mr-3">{selectedValue || label}</span>

      <span className="dropdown-arrow group-hover:rotate-180">
        <img src={arrowIcon} alt="arrow-icon" />
      </span>
    </button>
    <div
      className={`dropdown-menu ${isCalendar ? "dropdown-menu-calendar" : ""}`}
    >
      <div
        className={`dropdown-content ${
          hoverHighlight ? "region-highlight" : ""
        }`}
      >
        {children}
      </div>
    </div>
  </div>
);
