...
@router.get("/remediation-recommendations/summary")
def get_remediation_summary(
    region: Optional[str] = Query(None, description="Filter by region"),
    store_id: Optional[List[str]] = Query(None, description="Filter by store IDs"),
    risk_level: Optional[str] = Query(None, description="Filter by risk level"),
    db: Session = Depends(get_db)
):
    """
    Get summary statistics for remediation recommendations
    """
    try:
        from app.models.remediation_recommendations import RemediationRecommendation
        import pandas as pd
        ...
        query = db.query(RemediationRecommendation)
        
        if region:
            # Need to join with inventory to get region filter
            from app.models.inventory import Inventory
            query = query.join(Inventory, RemediationRecommendation.sku_id == Inventory.SKU_ID)
            query = query.filter(Inventory.Region_Historical == region)
        
        if store_id:
            query = query.filter(RemediationRecommendation.store_id.in_(store_id))
            
        if risk_level:
            query = query.filter(RemediationRecommendation.risk_level == risk_level)
        ...
        if not recommendations:
            return {
                ...
                "average_loss_mitigation_per_issue": 0
            }
        
        df = pd.DataFrame([r.__dict__ for r in recommendations])
        if '_sa_instance_state' in df.columns:
            df.pop('_sa_instance_state')
        
        # Calculate summary statistics
        total_issues = df['issue_id'].nunique() if 'issue_id' in df.columns else len(df)
        total_loss_mitigation = df['net_loss_mitigation'].fillna(0).sum()
        avg_loss_mitigation = total_loss_mitigation / total_issues if total_issues > 0 else 0
        ...
        return {
            ...
            "total_potential_loss_mitigation": round(total_loss_mitigation, 2),
            "risk_level_distribution": risk_distribution,
            "action_type_distribution": action_distribution,
            "average_loss_mitigation_per_issue": round(avg_loss_mitigation, 2)
        }
        
    except Exception as e:
        logger.error(f"Error getting remediation summary: {str(e)}")
        raise HTTPException(status_code=500, detail="Failed to get remediation summary")